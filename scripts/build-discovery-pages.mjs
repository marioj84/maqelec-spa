      ...
      <section class="machine-detail-hero">
        <div class="ma-container">
          ...
          <div class="machine-product-layout">
            <div class="machine-gallery"><div class="machine-gallery-main"><img src="${esc(gallery[0].src)}" alt="${esc(gallery[0].alt || item.imageAlt)}"></div></div>
            <div class="machine-product-copy"><div class="machine-product-labels"><small>${esc(item.category)}</small></div><h1>${esc(item.name)}</h1><p class="machine-product-summary">${esc(item.summary)}</p><div class="machine-key-facts">${facts}</div><div class="machine-commercial-note"><strong>Configuración a confirmar</strong><p>La propuesta se define según proceso, material, energía, accesorios y condiciones de instalación.</p></div><div class="hero-actions"><a class="button primary" href="cotizar.html?tipo=maquinaria&equipo=${encodeURIComponent(item.id)}">Cotizar este equipo</a><a class="button secondary" href="maquinaria.html">Ver maquinaria</a></div></div>
          </div>
        </div>
      </section>
...
function createProjectPage(item) {
  ...
  const output = `${head({ title: `${item.title} | Proyecto real MAQELEC`, description: item.summary, feature: "projects" })}
    <main id="mainContent">
      <section class="page-intro"><div class="ma-container"><span class="eyebrow">${esc(item.type)}</span><h1>${esc(item.title)}</h1><p>${esc(item.summary)}</p></div></section>
...
const staticMachines = catalog.machinery.map((item) => `<article class="catalog-card machine-card" data-searchable="${esc([item.name, item.category, item.summary, ...(item.specs || [])].join(" "))}"><div class="machine-visual machine-visual-photo"><img src="${esc(item.image)}" alt="${esc(item.imageAlt || item.name)}" loading="lazy"></div><div class="catalog-card-body"><div class="card-meta"><span>${esc(item.category)}</span></div><h3>${esc(item.name)}</h3><p>${esc(item.summary)}</p>${item.technicalSpecs ? `<a class="text-link" href="${slugMachine(item)}">Ver ficha técnica →</a>` : `<a class="text-link" href="cotizar.html?tipo=maquinaria&equipo=${encodeURIComponent(item.id)}">Consultar configuración →</a>`}</div></article>`).join("");

const staticServices = catalog.services.map((item) => `<article class="catalog-card service-card" data-searchable="${esc([item.name, item.category, item.summary].join(" "))}"><div class="service-image"><img src="${esc(item.image)}" alt="${esc(item.imageAlt || item.name)}" loading="lazy"></div><div class="catalog-card-body"><div class="card-meta"><span>${esc(item.category)}</span></div><h3>${esc(item.name)}</h3><p>${esc(item.summary)}</p><a class="text-link" href="${slugService(item)}">Ver servicio →</a></div></article>`).join("");

const staticProjects = catalog.projects.map((item) => `<article id="${esc(item.id)}" class="project-card"><div class="project-media"><img class="project-main-image" src="${esc(item.image)}" alt="${esc(item.imageAlt || item.title)}" loading="lazy"></div><div class="project-copy"><small>${esc(item.type)}</small><h3>${esc(item.title)}</h3><p>${esc(item.summary)}</p><a class="text-link" href="${slugProject(item)}">Abrir caso documentado →</a></div></article>`).join("");
